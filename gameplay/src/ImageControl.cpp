...
ImageControl::ImageControl() :
    _srcRegion(VRectangle::empty()), _dstRegion(VRectangle::empty()), _batch(NULL),
...
void ImageControl::setRegionSrc(const VRectangle& region)
{
    setRegionSrc(region.x, region.y, region.width, region.height);
}

const VRectangle& ImageControl::getRegionSrc() const
{
    return _srcRegion;
}
...
void ImageControl::setRegionDst(const VRectangle& region)
{
    setRegionDst(region.x, region.y, region.width, region.height);
}

const VRectangle& ImageControl::getRegionDst() const
{
    return _dstRegion;
}

unsigned int ImageControl::drawImages(Form* form, const VRectangle& clip)
{
    if (!_batch)
        return 0;
