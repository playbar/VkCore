    ...
    if (rec->trackStackTrace)
    {
        ...
        if (!initialized)
        {
            if (!SymInitialize(GetCurrentProcess(), NULL, true))
                vkcore::print("Stack trace tracking will not work.\n");
            initialized = true;
        }
...
void debugFree(void* p)
{
    ...
    {
        vkcore::print("[memory] CORRUPTION: Attempting to free memory address with invalid memory allocation record.\n");
        return;
    }
...
void printStackTrace(MemoryAllocationRecord* rec)
{
    ...
    for (int i = 0; i < MAX_STACK_FRAMES; i++)
    {
        ...
        {
            vkcore::print("[memory] STACK TRACE: <unknown location>\n");
        }
        else
        {
            ...
            else
            {
                ...
                if (pc != 0)
                {
                    ...
                    {
                        vkcore::print("[memory] STACK TRACE: %s - <unknown file>:<unknown line number>\n", symbol->Name);
                    }
                    else
                    {
                        ...
                        vkcore::print("[memory] STACK TRACE: %s - %s:%d\n", symbol->Name, file, line.LineNumber);
                    }
                }
            }
        }
...
extern void printMemoryLeaks()
{
    ...
    {
        vkcore::print("[memory] All HEAP allocations successfully cleaned up (no leaks detected).\n");
    }
    else
    {
        vkcore::print("[memory] WARNING: %d HEAP allocations still active in memory.\n", __memoryAllocationCount);
        ...
        while (rec)
        {
            ...
            if (rec->trackStackTrace)
            {
                vkcore::print("[memory] LEAK: HEAP allocation leak at address %#x of size %d:\n", rec->address, rec->size);
                printStackTrace(rec);
            }
            else
                vkcore::print("[memory] LEAK: HEAP allocation leak at address %#x of size %d from line %d in file '%s'.\n", rec->address, rec->size, rec->line, rec->file);
#else
            vkcore::print("[memory] LEAK: HEAP allocation leak at address %#x of size %d from line %d in file '%s'.\n", rec->address, rec->size, rec->line, rec->file);
#endif
            rec = rec->next;
