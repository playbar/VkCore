...
const VRectangle& Control::getBounds() const
{
    return _bounds;
}

void Control::setBounds(const VRectangle& bounds)
{
    setX(bounds.x);
    setY(bounds.y);
...
const VRectangle& Control::getAbsoluteBounds() const
{
    return _absoluteBounds;
}

const VRectangle& Control::getClipBounds() const
{
    return _clipBounds;
}

const VRectangle& Control::getClip() const
{
...
void Control::setSkinRegion(const VRectangle& region, unsigned char states)
{
    overrideStyle();
    Theme::Style::Overlay* overlays[Theme::Style::OVERLAY_MAX] = { 0 };
    getOverlays(states, overlays);
...
const VRectangle& Control::getSkinRegion(State state) const
{
    Theme::Style::Overlay* overlay = getOverlay(state);
    GP_ASSERT(overlay);
...
void Control::setImageRegion(const char* id, const VRectangle& region, unsigned char states)
{
    overrideStyle();
    Theme::Style::Overlay* overlays[Theme::Style::OVERLAY_MAX] = { 0 };
    getOverlays(states, overlays);
...
const VRectangle& Control::getImageRegion(const char* id, State state) const
{
    Theme::Style::Overlay* overlay = getOverlay(state);
    GP_ASSERT(overlay);
...
void Control::setCursorRegion(const VRectangle& region, unsigned char states)
{
    overrideStyle();
    Theme::Style::Overlay* overlays[Theme::Style::OVERLAY_MAX] = { 0 };
    getOverlays(states, overlays);
...
const VRectangle& Control::getCursorRegion(State state) const
{
    Theme::Style::Overlay* overlay = getOverlay(state);
    GP_ASSERT(overlay);
...
bool Control::updateBoundsInternal(const Vector2& offset)
{
    ...
    if (dirtyBounds)
    {
        ...
        VRectangle oldAbsoluteBounds(_absoluteBounds);
        VRectangle oldAbsoluteClipBounds(_absoluteClipBounds);
        VRectangle oldViewportBounds(_viewportBounds);
        VRectangle oldViewportClipBounds(_viewportClipBounds);
...
void Control::updateBounds()
{
    ...
    const VRectangle parentAbsoluteBounds = _parent ? _parent->_viewportBounds : VRectangle(0, 0, game->getViewport().width, game->getViewport().height);
    ...
    {
        const Theme::Margin& margin = _style->getMargin();
        const VRectangle& parentBounds = _parent ? _parent->getBounds() : VRectangle(0, 0, game->getViewport().width, game->getViewport().height);
        ...
        {
            const VRectangle& verticalScrollBarBounds = _parent->getImageRegion("verticalScrollBar", _parent->getState());
            const VRectangle& horizontalScrollBarBounds = _parent->getImageRegion("horizontalScrollBar", _parent->getState());
...
void Control::updateAbsoluteBounds(const Vector2& offset)
{
    ...
    const VRectangle parentAbsoluteBounds = _parent ? _parent->_viewportBounds : VRectangle(0, 0, game->getViewport().width, game->getViewport().height);
    const VRectangle parentAbsoluteClip = _parent ? _parent->_viewportClipBounds : parentAbsoluteBounds;
    ...
    VRectangle::intersect(_absoluteBounds, parentAbsoluteClip, &_absoluteClipBounds);
    ...
    VRectangle::intersect(_viewportBounds, parentAbsoluteClip, &_viewportClipBounds);
}
...
unsigned int Control::draw(Form* form, const VRectangle& clip)
{
    if (!_visible)
        return 0;
...
unsigned int Control::drawBorder(Form* form, const VRectangle& clip)
{
...
unsigned int Control::drawImages(Form* form, const VRectangle& position)
{
    return 0;
}

unsigned int Control::drawText(Form* form, const VRectangle& position)
{
    return 0;
}
