...
VulkanTextOverlay::~VulkanTextOverlay()
{
	...
	vkDestroyImage(mVulkanDevice->mLogicalDevice, image, nullptr);
	vkDestroyImageView(mVulkanDevice->mLogicalDevice, view, nullptr);
	vkFreeMemory(mVulkanDevice->mLogicalDevice, imageMemory, nullptr);
	...
	vkFreeCommandBuffers(mVulkanDevice->mLogicalDevice, commandPool, static_cast<uint32_t>(cmdBuffers.size()), cmdBuffers.data());
	vkDestroyCommandPool(mVulkanDevice->mLogicalDevice, commandPool, nullptr);
...
void VulkanTextOverlay::prepareResources()
{
	...
	VK_CHECK_RESULT(vkCreateCommandPool(mVulkanDevice->mLogicalDevice, &cmdPoolInfo, nullptr, &commandPool));
	...
	VK_CHECK_RESULT(vkAllocateCommandBuffers(mVulkanDevice->mLogicalDevice, &cmdBufAllocateInfo, cmdBuffers.data()));
	...
	VK_CHECK_RESULT(vkCreateImage(mVulkanDevice->mLogicalDevice, &imageInfo, nullptr, &image));

	VkMemoryRequirements memReqs;
	VkMemoryAllocateInfo allocInfo = vkTools::initializers::memoryAllocateInfo();
	vkGetImageMemoryRequirements(mVulkanDevice->mLogicalDevice, image, &memReqs);
	allocInfo.allocationSize = memReqs.size;
	allocInfo.memoryTypeIndex = mVulkanDevice->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
	VK_CHECK_RESULT(vkAllocateMemory(mVulkanDevice->mLogicalDevice, &allocInfo, nullptr, &imageMemory));
	VK_CHECK_RESULT(vkBindImageMemory(mVulkanDevice->mLogicalDevice, image, imageMemory, 0));
	...
	vkFreeCommandBuffers(mVulkanDevice->mLogicalDevice, commandPool, 1, &copyCmd);
	...
	VK_CHECK_RESULT(vkCreateImageView(mVulkanDevice->mLogicalDevice, &imageViewInfo, nullptr, &view));
	...
	VK_CHECK_RESULT(vkCreateSampler(mVulkanDevice->mLogicalDevice, &samplerInfo, nullptr, &sampler));
	...
	VK_CHECK_RESULT(vkCreateDescriptorSetLayout(mVulkanDevice->mLogicalDevice, &descriptorSetLayoutInfo, nullptr, &descriptorSetLayout));
	...
	VK_CHECK_RESULT(vkCreatePipelineLayout(mVulkanDevice->mLogicalDevice, &pipelineLayoutInfo, nullptr, &pipelineLayout));
	...
	VK_CHECK_RESULT(vkAllocateDescriptorSets(mVulkanDevice->mLogicalDevice, &descriptorSetAllocInfo, &descriptorSet));
...
void VulkanTextOverlay::preparePipeline()
{
	...
	VK_CHECK_RESULT(vkCreateGraphicsPipelines(mVulkanDevice->mLogicalDevice, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipeline));
}
