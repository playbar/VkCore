...
namespace vkTools
{
	...
	VulkanTextureLoader::VulkanTextureLoader(VkCoreDevice *vulkanDevice, VkQueue queue, VkCommandPool cmdPool)
	{
	...
	*/
	void VulkanTextureLoader::loadTexture(std::string filename, VkFormat format, VulkanTexture *texture, bool forceLinear = false, VkImageUsageFlags imageUsageFlags)
	{
#if defined(__ANDROID__)
	...
	* @note Only supports .ktx and .dds
	*/
	void VulkanTextureLoader::loadCubemap(std::string filename, VkFormat format, VulkanTexture *texture, VkImageUsageFlags imageUsageFlags)
	{
#if defined(__ANDROID__)
		assert(assetManager != nullptr);
	...
	* @note Only supports .ktx and .dds
	*/
	void VulkanTextureLoader::loadTextureArray(std::string filename, VkFormat format, VulkanTexture *texture, VkImageUsageFlags imageUsageFlags)
	{
#if defined(__ANDROID__)
		assert(assetManager != nullptr);
	...
	void VulkanTextureLoader::createTexture(void* buffer, VkDeviceSize bufferSize, VkFormat format,
		uint32_t width, uint32_t height, VulkanTexture *texture,
