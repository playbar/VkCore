...
public:
	...
	void prepareNoiseTexture(uint32_t width, uint32_t height, uint32_t depth)
	{
		...
		uint32_t maxImageDimension3D(mVulkanDevice->mProperties.limits.maxImageDimension3D);
...
#define FRACTAL
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < mDrawCmdBuffers.size(); ++i)
		{
			...
			vkCmdBindVertexBuffers(mDrawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1, &vertexBuffer.buffer, offsets);
			vkCmdBindIndexBuffer(mDrawCmdBuffers[i], indexBuffer.buffer, 0, VK_INDEX_TYPE_UINT32);
			vkCmdDrawIndexed(mDrawCmdBuffers[i], indexCount, 1, 0, 0, 0);
